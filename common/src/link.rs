...
async fn link_internal(
    ...
    // linked/copied into it. if nothing was linked, we should clean up the empty directory we
    // created (or not count it in dry-run mode). this prevents empty directories from being
    // created when they were only traversed to look for potential matches.
    // NOTE: we only clean up if WE created the directory - if it already existed (--overwrite),
    // we should not remove it even if nothing was linked into it.
