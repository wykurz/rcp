...
fn test_remote_auto_deploy_rcpd() {
    ...
    // note: we don't hide local rcpd binaries anymore since we use --rcpd-path=/nonexistent
    // to force the discovery to fail and trigger auto-deployment. This allows the deployment
    // to find the correct local rcpd binary (same build as rcp) to transfer.

    // run with --auto-deploy-rcpd and explicit non-existent path to force deployment
    // note: we use --rcpd-path to force discovery failure since we can't reliably
    // hide all rcpd binaries (nix profile is owned by root and can't be renamed)
    eprintln!(
