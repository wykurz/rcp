...
struct Args {
    ...
    ///
    /// Refuses non-Linux builds (rlink has no --dereference), kernels without openat2
    /// (Linux 5.6+), and any operand — src, dst, or --update — that is not absolute and
    /// lexically normal (no `.`/`..`/`//`; realpath output qualifies). Operand root opens
    /// then resolve with openat2(RESOLVE_NO_SYMLINKS), so a symlink in any directory component
    /// fails closed (a symlink operand itself is never followed — it is handled as the link
    /// object). Path POLICY stays the caller's: lock paths down in the sudo rule. See
    /// "Scope of TOCTOU safety" in docs/tocttou.md. Intended for sudo rules:
    /// `NOPASSWD: /usr/bin/rlink --require-toctou-safe *`.
